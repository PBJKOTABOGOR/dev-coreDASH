...
(function(){
  ...
  const MODULE = 'ekatalog';
  const DATASET = '';
  const TITLE = 'Monitoring eKatalog Cepat';
  const SUBTITLE = 'Mode cepat nasional untuk pemanfaatan eKatalog dengan backend-side search.';
  const COLUMNS = [{"label": "Kode Paket", "keys": ["kode_paket", "kode", "kode_rup"]}, {"label": "Nama Paket", "keys": ["nama_paket", "nama_produk"]}, {"label": "OPD", "keys": ["satuan_kerja", "satker", "opd"]}, {"label": "Penyedia", "keys": ["nama_penyedia", "penyedia", "nama_pemenang"]}, {"label": "Status", "keys": ["status_paket", "status", "tahap_aktif"], "type": "badge"}, {"label": "Nilai", "keys": ["total_nilai", "nilai_paket", "pagu", "hps"], "type": "money"}, {"label": "Tahun", "keys": ["tahun"]}];
  const SUMMARY_MAP = {"fastStat1": {"label": "Jumlah OPD", "keys": ["jumlahOpd", "instansi"]}, "fastStat2": {"label": "Jumlah Paket", "keys": ["jumlahPaket", "totalRows"]}, "fastStat3": {"label": "Total Nilai", "keys": ["totalPagu", "totalRealisasi"], "type": "money"}, "fastStat4": {"label": "Rata-rata ITKP", "keys": ["avgItkp"], "type": "decimal"}};
