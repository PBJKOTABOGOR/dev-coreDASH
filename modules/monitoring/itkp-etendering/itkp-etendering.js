...
(function(){
  ...
  const MODULE = 'etendering';
  const DATASET = '';
  const TITLE = 'Monitoring eTendering Cepat';
  const SUBTITLE = 'Tabel eTendering tidak lagi mengirim semua baris ke browser.';
  const COLUMNS = [{"label": "Kode Tender", "keys": ["kode_tender", "kode_paket", "kode"]}, {"label": "Kode RUP", "keys": ["kode_rup"]}, {"label": "Nama Paket", "keys": ["nama_paket"]}, {"label": "OPD", "keys": ["satuan_kerja", "satker", "opd"]}, {"label": "Metode", "keys": ["metode", "metode_pemilihan"]}, {"label": "Tahap", "keys": ["tahap", "tahap_aktif", "status"], "type": "badge"}, {"label": "Pagu/HPS", "keys": ["pagu", "hps"], "type": "money"}, {"label": "Pemenang", "keys": ["pemenang", "nama_pemenang"]}];
  const SUMMARY_MAP = {"fastStat1": {"label": "Jumlah OPD", "keys": ["jumlahOpd", "instansi"]}, "fastStat2": {"label": "Jumlah Paket", "keys": ["jumlahPaket", "totalRows"]}, "fastStat3": {"label": "Total Pagu/HPS", "keys": ["totalPagu"], "type": "money"}, "fastStat4": {"label": "Rata-rata ITKP", "keys": ["avgItkp"], "type": "decimal"}};
