...
(function(){
  ...
  const MODULE = 'ekontrak';
  const DATASET = '';
  const TITLE = 'Monitoring eKontrak Cepat';
  const SUBTITLE = 'Monitoring kontrak dengan summary, search, dan paging dari backend.';
  const COLUMNS = [{"label": "Kode RUP", "keys": ["kode_rup", "kd_rup"]}, {"label": "Kode Paket", "keys": ["kode_paket", "kode"]}, {"label": "Nama Paket", "keys": ["nama_paket"]}, {"label": "OPD", "keys": ["satuan_kerja", "satker", "opd"]}, {"label": "Penyedia", "keys": ["penyedia", "nama_penyedia", "nama_pemenang"]}, {"label": "Status Kontrak", "keys": ["status_kontrak", "status"], "type": "badge"}, {"label": "Nilai Kontrak", "keys": ["nilai_kontrak", "kontrak"], "type": "money"}];
  const SUMMARY_MAP = {"fastStat1": {"label": "Jumlah OPD", "keys": ["jumlahOpd", "instansi"]}, "fastStat2": {"label": "Jumlah Paket", "keys": ["jumlahPaket", "totalRows"]}, "fastStat3": {"label": "Nilai Kontrak", "keys": ["totalRealisasi", "totalPagu"], "type": "money"}, "fastStat4": {"label": "Rata-rata ITKP", "keys": ["avgItkp"], "type": "decimal"}};
